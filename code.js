...
const HUBSPOT_SERVERLESS_ENDPOINT =
	"https://www.activesgcircle.gov.sg/_hcms/api/update-data-sport-pickleball";
...
const TARGET_SHEET_NAME = "Pesta Sukan"; // poduction sheet
/**
...
function findRowsNeedingHubDBCreation(allData, sheet, changes) {
	...
	changedUniqueIds.forEach((uniqueId) => {
		...
		const hasRequiredData =
			data.date_and_time && data.player_team_1 && data.player_team_2;
		...
		for (const rowInfo of rowsNeedingCreation) {
			...
			const payload = {
				...
				data: {
					...
					player_team_1: rowInfo.data.player_team_1,
					player_team_2: rowInfo.data.player_team_2,
					winner: rowInfo.data.winner,
				},
				metadata: {
